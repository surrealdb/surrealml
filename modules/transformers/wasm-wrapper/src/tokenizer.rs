...
use surrealml_tokenizers::{
    Tokenizer,
    load_tokenizer as load_tokenizer_surrealml,
    encode as encode_surrealml,
    decode as decode_surrealml,
};
use crate::utils::{
    StringReturn,
    TokenizerReturn,
...
use crate::{
    process_string_for_tokenizer_return,
    process_opt_string_for_tokenizer_return,
...
pub extern "C" fn encode(
    ...
    if tokenizer_handle.is_null() {
        return VecU32Return::error(format!("Received null pointer for tokenizer handle in encode fn"));
    }
...
pub extern "C" fn decode(
    ...
    if tokenizer_handle.is_null() {
        return StringReturn {
            ...
            is_error: 1,
            error_message: CString::new("Received null pointer for tokenizer handle").unwrap().into_raw(),
        }
    }
    let tokenizer = unsafe { &(*tokenizer_handle).tokenizer };
    ...
    match decode_surrealml(tokenizer, slice) {
        Ok(decoded_string) => {
            match CString::new(decoded_string) {
                ...
                Err(_) => return StringReturn {
                    string: std::ptr::null_mut(),
                    is_error: 1,
                    error_message: CString::new("Failed to create CString from decoded string").unwrap().into_raw(),
                },
            }
