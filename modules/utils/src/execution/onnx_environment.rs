...
pub static LIB_BYTES: &'static [u8] = include_bytes!("../../target/debug/libonnxruntime.dylib");

#[cfg(all(any(target_os = "linux", target_os = "android"), not(doc), not(onnx_runtime_env_var_set)))]
pub static LIB_BYTES: &'static [u8] = include_bytes!("../../target/debug/libonnxruntime.so");

#[cfg(all(target_os = "windows", not(doc), not(onnx_runtime_env_var_set)))]
pub static LIB_BYTES: &'static [u8] = include_bytes!("../../target/debug/onnxruntime.dll");
