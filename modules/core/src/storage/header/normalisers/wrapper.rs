...
impl Display for NormaliserType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            NormaliserType::LinearScaling(linear_scaling) => write!(
                f,
                "linear_scaling({},{})",
                linear_scaling.min, linear_scaling.max
            ),
            NormaliserType::Clipping(clipping) => write!(
                f,
                "clipping({},{})",
                clipping.min.unwrap(),
                clipping.max.unwrap()
            ...
            NormaliserType::LogScaling(log_scaling) => {
                write!(f, "log_scaling({},{})", log_scaling.base, log_scaling.min)
            }
            NormaliserType::ZScore(z_score) => {
                write!(f, "z_score({},{})", z_score.mean, z_score.std_dev)
            }
        }
    }
}
