...
impl SurMlFile {
    ...
    /// # Returns
    /// A new `SurMlFile` struct.
    pub fn from_bytes(bytes: Vec<u8>) -> Result<Self, NanoServiceError> {
    ...
    /// # Returns
    /// A new `SurMlFile` struct.
    pub fn from_file(file_path: &str) -> Result<Self, NanoServiceError> {
        let mut file = safe_eject!(File::open(file_path), NanoServiceErrorStatus::NotFound)?;

        // extract the first 4 bytes as an integer to get the length of the header
        let mut buffer = [0u8; 4];
        safe_eject!(file.read_exact(&mut buffer), NanoServiceErrorStatus::BadRequest)?;
        let integer_value = u32::from_be_bytes(buffer);

        // Read the next integer_value bytes for the header
        let mut header_buffer = vec![0u8; integer_value as usize];
        safe_eject!(file.read_exact(&mut header_buffer), NanoServiceErrorStatus::BadRequest)?;

        // Create a Vec<u8> to store the data
        let mut model_buffer = Vec::new();

        // Read the rest of the file into the buffer
        safe_eject!(file.take(usize::MAX as u64).read_to_end(&mut model_buffer), NanoServiceErrorStatus::BadRequest)?;
    ...
    pub fn write(&self, file_path: &str) -> Result<(), NanoServiceError> {
